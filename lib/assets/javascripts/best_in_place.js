...
BestInPlaceEditor.prototype = {
  ...
  initOptions : function() {
    ...
    self.okButtonClass     = self.element.attr("data-ok-button-class")     || self.okButtonClass || "btn btn-primary btn-sm";
    self.cancelButton      = self.element.attr("data-cancel-button")       || self.cancelButton;
    self.cancelButtonClass = self.element.attr("data-cancel-button-class") || self.cancelButtonClass || "btn btn-default cancel btn-sm";
    ...
    self.cancel_on_blur   = self.element.attr("data-cancel-on-blur")    || true;
  ...
  setHtmlAttributes : function() {
    var selector = this.formType.match(/(number|date|time)/gi) ? "input" : this.formType;
    var formField = this.element.find(selector);
...
BestInPlaceEditor.forms = {
  "input" : {
    activateForm : function() {
      ...
      if (this.cancel_on_blur == true || this.cancel_on_blur == undefined) {
  ...
  "number" : {
    activateForm : function() {
      ...
      this.element.find("form").bind('submit', {editor: this}, BestInPlaceEditor.forms.number.submitHandler);
      if (this.cancelButton) {
        this.element.find("input[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.number.cancelButtonHandler);
      }
      if (this.cancel_on_blur == true || this.cancel_on_blur == undefined) {
        this.element.find("input[type='text']").bind('blur', {editor: this}, BestInPlaceEditor.forms.input.inputBlurHandler);
      }
      this.element.find("input[type='number']").bind('keyup', {editor: this}, BestInPlaceEditor.forms.number.keyupHandler);
      this.blurTimer = null;
      this.userClicked = false;
    },
  ...
  "time" : {
    activateForm : function() {
      ...
      output.append(input_elt);
      var remove_btn_elt = jQuery(document.createElement('span'))
                      .attr('class', 'icon icon-large icon-remove-sign')
                      .click(function(event) {
                        $(this).parent().find('input[type=text]').val('');
                      });
      output.append(remove_btn_elt);
      if(this.okButton) {
        output.append(
          jQuery(document.createElement('input'))
          .attr('type', 'submit')
      ...
      this.element.html(output);
      this.setHtmlAttributes();
      var date_time = this.element.find("input[type='text']")

      var time = "";
      if (result = date_time.val().match(/\d{1,2}:\d\d$/)){
        time = result[0];
      }

      this.element.find("input[type='text']")
        .timepicker({
            'timeFormat': 'H:i',
            'step': 30,
          });
      this.element.find("input[type='text']").timepicker("setTime",time);
      this.element.find("input[type='text']").timepicker("show");

      this.element.find("input[type='text']")[0].select();
      this.element.find("form").bind('submit', {editor: this}, BestInPlaceEditor.forms.time.submitHandler);
      if (this.cancelButton) {
        this.element.find("input[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.time.cancelButtonHandler);
      }
      //temporary solution to make possible save new time
      //this.element.find("input[type='text']").bind('blur', {editor: this}, BestInPlaceEditor.forms.time.inputBlurHandler);
      this.element.find("input[type='text']").bind('keyup', {editor: this}, BestInPlaceEditor.forms.time.keyupHandler);
      this.element.find("input[type='text']").bind('touchstart', function(event){
        event.preventDefault();
        $(event.target).timepicker("show");
      });
      this.blurTimer = null;
      this.userClicked = false;
    },
